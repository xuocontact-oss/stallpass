...
export default async function AccountPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-lg space-y-5 px-4 py-6">
      <h1 className="text-2xl font-bold">{t("Your account")}</h1>
      ...
      <section className="space-y-2">
        <h2 className="font-semibold">{t("Your market reviews")}</h2>
        {(reviews ?? []).length === 0 ? (
          <p className="rounded-xl border bg-background p-4 text-sm text-muted-foreground">
            {t("None yet. Visited a market?")}{" "}
            <Link href="/markets" className="text-primary">{t("Find it and leave a review")}</Link>.
          </p>
        ) : (
            ...
            {(reviews as unknown as { id: string; visited_on: string; rating_overall: number; is_hidden: boolean; hidden_reason: string | null; markets: { name: string; slug: string } }[]).map((r) => (
              <li key={r.id} className="flex flex-wrap items-center gap-3 p-4 text-sm">
                <Link href={`/markets/${r.markets.slug}/review`} className="min-w-0 flex-1 hover:underline">
                  ...
                  <p className="text-muted-foreground">{t("Visited {date}", { date: formatDate(r.visited_on, { lang }) })}</p>
                  {r.is_hidden && <p className="text-destructive">{t("Hidden by Stallpass")}{r.hidden_reason ? `: ${r.hidden_reason}` : ""}</p>}
                </Link>
                <Stars value={r.rating_overall} />
                <ConfirmButton size="sm" variant="ghost" action={deleteShopperReview.bind(null, r.id)} confirmText={t("Delete this review?")}>
                  {t("Delete")}
                </ConfirmButton>
              </li>
            ))}
      ...
      <section className="space-y-3 rounded-xl border bg-background p-4">
        <h2 className="font-semibold">{t("Your data")}</h2>
        ...
        <details className="rounded-lg border border-destructive/30 p-3">
          <summary className="cursor-pointer text-sm font-medium text-destructive">{t("Delete my account")}</summary>
          <ActionForm action={deleteMyAccount} className="mt-3 space-y-3">
            <p className="text-sm">
              {t("This permanently deletes your account, business profile, documents, applications and reviews. It can't be undone. Markets you run stay listed without you.")}
            </p>
            <div className="space-y-1.5">
              <Label htmlFor="confirm">{t("Type DELETE to confirm")}</Label>
