  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-xs text-muted-foreground">
        <span>
          Step {step} of 4 · <span className="font-medium text-foreground">{STEP_NAMES[step - 1]}</span>
        </span>
      </div>
  ...
  if (!vendor && !requested) {
    return (
      <main className="mx-auto w-full max-w-lg space-y-5 px-4 py-8">
        <div>
          <h1 className="text-2xl font-bold">Welcome to Stallpass!</h1>
        ...
        <Link href="/onboarding?step=1" className="flex items-center gap-4 rounded-xl border-2 bg-background p-4 hover:border-primary">
          ...
          <span>
            <span className="block font-semibold">I sell at markets</span>
            <span className="text-sm text-muted-foreground">Food, drinks, clothing, crafts, art… Set up your vendor profile.</span>
          </span>
        </Link>
        <Link href="/organizer/start" className="flex items-center gap-4 rounded-xl border-2 bg-background p-4 hover:border-primary">
          ...
          <span>
            <span className="block font-semibold">I run a market</span>
            <span className="text-sm text-muted-foreground">Claim or add your market and manage vendor applications.</span>
          </span>
        </Link>
        <Link href="/onboarding/shopper" className="flex items-center gap-4 rounded-xl border-2 bg-background p-4 hover:border-primary">
          ...
          <span>
            <span className="block font-semibold">I&apos;m a shopper</span>
            <span className="text-sm text-muted-foreground">Find farmers markets and night markets near you, and share reviews.</span>
          </span>
        </Link>
        {profile?.is_organizer && (
          <p className="text-center text-sm">
            <Link href="/organizer" className="text-primary">Back to your markets</Link>
          </p>
        )}
      </main>
    )
  ...
  return (
    <main className="mx-auto w-full max-w-lg space-y-5 px-4 py-6">
      ...
      {step === 1 && (
        <>
          <div>
            <h1 className="text-2xl font-bold">{vendor ? "You & your business" : "Let's set up your business"}</h1>
            <p className="text-sm text-muted-foreground">
              Markets see this when you apply.{" "}
              <Link href="/start" className="font-medium text-primary">
                First time selling at markets? Check out our quick start guide →
              </Link>
            </p>
          </div>
          <ActionForm action={saveVendorBasics} className="space-y-5 rounded-xl border bg-background p-4">
            ...
            <fieldset className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">Shared with markets you apply to, so they can reach you on the day.</p>
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="email">Email</Label>
                <Input id="email" value={user.email} disabled readOnly />
                <p className="text-xs text-muted-foreground">Your login. Markets reply to this address.</p>
              </div>
            </fieldset>
            <fieldset className="space-y-4 border-t pt-4">
              <legend className="mb-2 pt-4 font-semibold">Your business</legend>
      ...
      {step === 2 && vendor && (
        <>
          <div>
            ...
            <p className="text-sm text-muted-foreground">A good description and photos get you accepted more often.</p>
          </div>
          <section className="space-y-3 rounded-xl border bg-background p-4">
            <div>
              <h2 className="font-semibold">Photos</h2>
              <p className="text-sm text-muted-foreground">
                Your booth or truck, your {isFoodCategory(vendor.category) ? "food" : "products"}, your setup.
              </p>
            </div>
            <PhotoManager
          ...
          <ActionForm action={saveVendorDetails} className="space-y-5 rounded-xl border bg-background p-4">
            ...
            />
            <div className="space-y-1.5">
              <Label htmlFor="setup_notes">Setup details (optional)</Label>
              <Input id="setup_notes" name="setup_notes" maxLength={500} defaultValue={vendor.setup_notes ?? ""} placeholder="e.g. 10x10 tent, 2 tables, needs one outlet" />
            </div>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="service_area">Where you&apos;ll travel</Label>
              <Input id="service_area" name="service_area" maxLength={200} placeholder="e.g. LA County, up to 30 miles from Echo Park" defaultValue={vendor.service_area ?? ""} />
            </div>
            <div className="flex items-center gap-3">
              <Link href="/onboarding?step=1" className={buttonVariants({ variant: "ghost" })}>Back</Link>
              <SubmitButton size="lg" className="flex-1" pendingText="Saving…">
                Save and continue
              </SubmitButton>
            </div>
          </ActionForm>
          <p className="text-center text-sm">
            <Link href="/onboarding?step=3" className="text-muted-foreground">Skip for now</Link>
          </p>
        </>
      )}
      ...
      {step === 3 && vendor && (
        <>
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Upload what you have now. They stay private until you apply, and we&apos;ll remind you before anything expires.
            </p>
          </div>
          ...
          <p className="text-sm text-muted-foreground">
            Something else a market asked for (fire inspection, etc.)? Add it any time under <span className="font-medium">Docs</span>.
          </p>
          <div className="flex items-center gap-3">
            <Link href="/onboarding?step=2" className={buttonVariants({ variant: "ghost" })}>Back</Link>
            <Link href="/onboarding?step=4" className={buttonVariants({ size: "lg", className: "flex-1" })}>
              {progress.docsDone === progress.docsTotal ? "Continue" : "Continue (finish later)"}
            </Link>
          </div>
        </>
      )}
      ...
      {step === 4 && vendor && (
        <>
          <div>
            <h1 className="text-2xl font-bold">{progress.complete ? "You're all set! 🎉" : "Nearly there"}</h1>
            <p className="text-sm text-muted-foreground">
              {progress.complete
                ? "Your profile and documents are ready. Time to find markets."
                : "You can start browsing markets now and finish the rest any time."}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Link href="/markets" className={buttonVariants({ size: "lg", className: "flex-1" })}>Find markets</Link>
